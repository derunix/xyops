...
class Jobs {
	...
	launchJob(job, callback) {
		...
		// save original param templates for re-expansion after start actions
		if (job.params) {
			job._paramTemplates = {};
			for (var key in job.params) {
				if (typeof(job.params[key]) == 'string' && job.params[key].includes('{{')) {
					job._paramTemplates[key] = job.params[key];
				}
			}
		}
		...
		switch (job.state) {
			...
			case 'ready':
				...
				this.runJobActions(job, 'start', function() {
					...
					// re-expand saved {{ }} param templates with data from start actions
					// (start actions like bucket_fetch may have populated input.data)
					var info = self.jobDetails[ job.id ];
					if (job._paramTemplates && info && info.input && info.input.data) {
						var context = { ...job };
						context.data = info.input.data;
						if (info.input.files) context.files = info.input.files;

						for (var key in job._paramTemplates) {
							job.params[key] = self.messageSub( job._paramTemplates[key], context );
						}
						delete job._paramTemplates;
					}
